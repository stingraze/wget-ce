/* mini_wget_ce.c - Windows CE compatible version */
#define WIN32_LEAN_AND_MEAN
#include <winsock.h>      /* Use winsock.h for Windows CE */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

/* Build: arm-mingw32ce-gcc -O2 -DWINCE mini_wget_ce.c -o mini-wget.exe -lwsock32 */
...
/* URL parser remains the same */
static int split_url(const char *url, char *host, size_t hsz, char *path, size_t psz, int *port)
{
...
static int http_get(const char *host, const char *path, int port, FILE *out)
{
    ...
    /* Use Winsock 1.1 for Windows CE */
    if (WSAStartup(MAKEWORD(1,1), &wsa)) { perror("WSAStartup"); return 1; }
...
int main(int argc, char *argv[])
{
    ...
    int binary_mode = 1; /* Default to binary mode */
    const char *url = NULL;
    FILE *out = stdout;
    int i;
